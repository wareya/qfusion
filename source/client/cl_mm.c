...
void CL_MM_Heartbeat( void ) {
	...
	if( !cl_mm_enabled || !Uuid_IsValidAuthSessionId( cls.mm_session ) ) {
		return;
	}
...
static void cl_mm_login_done( stat_query_t *query, bool success, void *customp ) {
	...
	if( root == NULL ) {
		...
		if( rstatus == LOGIN_RESPONSE_NONE ) {
			...
			if( Uuid_IsValidAuthSessionId( cls.mm_session ) && ratings_section != NULL ) {
				int idx = 0;
	...
	if( cl_mm_loginState == LOGIN_STATE_NONE ) {
		...
		cl_mm_enabled = Uuid_IsValidAuthSessionId( cls.mm_session );
		if( cl_mm_enabled ) {
