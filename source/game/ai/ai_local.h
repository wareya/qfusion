...
// These shorthands are for backward compatibility and some degree of convenience
...
inline void SolidAndLiquidWorldTrace( trace_t *trace, const vec3_t from, const vec3_t to,
									  const vec3_t mins = vec3_origin, const vec3_t maxs = vec3_origin ) {
	StaticWorldTrace( trace, from, to, MASK_SOLID | MASK_WATER, mins, maxs );
}
