...
BotMovementFallback *BotDummyMovementAction::TryFindMovementFallback( BotMovementPredictionContext *context ) {
	...
	if( context->NavTargetOrigin().SquareDistanceTo( entityPhysicsState.Origin() ) < SQUARE( 384.0f ) ) {
		nodeFallback->Activate( context->NavTargetOrigin().Data(), 32.0f, context->NavTargetAasAreaNum() );
...
BotMovementFallback *BotDummyMovementAction::TryFindAasBasedFallback( BotMovementPredictionContext *context ) {
	...
	const auto &entityPhysicsState = context->movementState->entityPhysicsState;
	const auto *aasWorld = AiAasWorld::Instance();
	const auto &nextReach = aasWorld->Reachabilities()[nextReachNum];
	const int traveltype = nextReach.traveltype & TRAVELTYPE_MASK;

	if( traveltype == TRAVEL_WALK ) {
		// Allow following WALK reachabilities but make sure the do not lead to junk areas
		float squareDistance = DistanceSquared( entityPhysicsState.Origin(), nextReach.end );
		if( squareDistance > SQUARE( 56.0f ) ) {
			const auto &area = aasWorld->Areas()[nextReach.areanum];
			if( area.maxs[0] - area.mins[0] > 28.0f && area.maxs[1] - area.mins[1] > 28.0f ) {
				...
				fallback->Activate( nextReach.end, 16.0f, nextReach.areanum, timeout );
				return fallback;
			}
		}
		return nullptr;
	...
	if( traveltype == TRAVEL_JUMPPAD || traveltype == TRAVEL_TELEPORT || traveltype == TRAVEL_ELEVATOR ) {
		...
		auto *fallback = &self->ai->botRef->useWalkableNodeMovementFallback;
		fallback->Activate( nextReach.start, 16.0f, nextReach.areanum );
		return fallback;
	}
	...
	if( traveltype == TRAVEL_WALKOFFLEDGE ) {
		...
		if( squareFallingHeight < SQUARE( 40.0f ) ) {
			...
			fallback->Activate( nextReach.start, 16.0f, nextReach.areanum, timeout );
			return fallback;
		}
		...
		targetOrigin.Z() = aasWorld->Areas()[nextReach.areanum].mins[2] + 4.0f - playerbox_stand_mins[2];
...
bool BotGenericGroundMovementFallback::TestActualWalkability( int targetAreaNum, const vec3_t targetOrigin,
	...
	if( trace.fraction != 1.0f && Distance2DSquared( targetOrigin, trace.endpos ) > 8 * 8 ) {
		return false;
	}
...
void BotJumpToFeasibleSpotMovementFallback::Activate( const vec3_t startOrigin_,
...
bool BotJumpToFeasibleSpotMovementFallback::TryDeactivate( BotMovementPredictionContext *context ) {
	assert( status == PENDING );
...
void BotJumpToFeasibleSpotMovementFallback::SetupMovement( BotMovementPredictionContext *context ) {
	const auto &entityPhysicsState = context->movementState->entityPhysicsState;
	auto *botInput = &context->record->botInput;
	...
	if( toTargetDir.Dot( entityPhysicsState.ForwardDir() ) < 0.95f ) {
...
static bool PredictTrajectoryHit( vec3_t origin, const vec3_t velocity,
	...
	for( unsigned i = 1; i <= maxSteps; ++i ) {
		...
		if( velocity[2] - gravity * t >= 0 ) {
			SolidWorldTrace( &trace, prevOrigin, origin, mins, maxs );
		} else {
			SolidAndLiquidWorldTrace( &trace, prevOrigin, origin, mins, maxs );
		}
...
static const float *FindBestJumpableSpot( BotMovementPredictionContext *context,
	...
	while( spotsBegin != spotsEnd ) {
		...
		if( PredictTrajectoryHit( predictedOrigin, velocity.Data(), mins, maxs, 96, 6, &hitContents, tmpPlane.normal ) ) {
			...
			traceEnd.Z() -= 64.0f;
			SolidAndLiquidWorldTrace( &trace, predictedOrigin, traceEnd.Data() );
			if( trace.fraction == 1.0f ) {
				continue;
			}
			if( !ISWALKABLEPLANE( &trace.plane ) ) {
...
BotMovementFallback *BotDummyMovementAction::TryFindJumpFromLavaFallback( BotMovementPredictionContext *context ) {
	...
	const auto &entityPhysicsState = context->movementState->entityPhysicsState;
	auto *const fallback = &self->ai->botRef->jumpToFeasibleSpotMovementFallback;
